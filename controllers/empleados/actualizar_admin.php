...
if ($body = json_decode(file_get_contents('php://input'))) {
	if (
		...
		if (isset($empleado->password)) {
			$sql = 'UPDATE empleados e SET e.email = \''.$empleado->email.'\', e.password = \''.$empleado->password.'\', e.nombre = \''.$empleado->nombre.'\', e.apellido = \''.$empleado->apellido.'\', e.fecha_nacimiento = \''.$empleado->fecha_nacimiento.'\', e.img_perfil = \''.$empleado->img_perfil.'\', e.role = \''.$empleado->role->id.'\' WHERE e.id = \''.$empleado->id.'\'';
		} else {
